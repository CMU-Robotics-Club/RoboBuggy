 ...
 * CHANGELOG: NONE
 * 
 * DESCRIPTION:  where all system configuration value are set if a json config file is provided 
 *               then values in json are used instead of the default values, this is intended
 *               to allow for easy deployment to different computers and to allow for different
 *               modes ie TestSensors, LogData, raceday config ...
 */
...
/**
 * 
 * @param filename(the file to read json from)
 */
public static void Set(String filename) {
	System.out.println("Setting configuration parameters from file: " + filename);
	
	try {
		JSONParser parser = new JSONParser();
		JSONObject obj = (JSONObject)parser.parse(new FileReader(filename));
		
		FRONT_CAM_INDEX  = (int)obj.getOrDefault("FRONT_CAM_INDEX", config.FRONT_CAM_INDEX);
		REAR_CAM_INDEX = (int)obj.getOrDefault("BACK_CAM_INDEX", config.REAR_CAM_INDEX);
		VISION_SYSTEM_EXECUTABLE_LOCATION = (String)obj.getOrDefault("VISION_SYSTEM_EXECUTABLE_LOCATION",
		...
		AUTONOMUS_DEFAULT = (boolean)obj.getOrDefault("AUTONOMOUS", config.AUTONOMUS_DEFAULT);
		DRIVE_DEFAULT = (boolean)obj.getOrDefault("DRIVE", config.DRIVE_DEFAULT);
		
		IMU_DEFAULT = (boolean)obj.getOrDefault("IMU", config.IMU_DEFAULT);
		GPS_DEFAULT = (boolean)obj.getOrDefault("GPS", config.GPS_DEFAULT);
		ENCODER_DEFAULT = (boolean)obj.getOrDefault("ENCODER", config.ENCODER_DEFAULT);
		VISION_SYSTEM_DEFAULT = (boolean)obj.getOrDefault("VISION", config.VISION_SYSTEM_DEFAULT);
		BRAKES_PER_FULL_PRESSURE = (byte)obj.getOrDefault("BRAKE_PER_FULL_PRESSURE", 
				config.BRAKES_PER_FULL_PRESSURE);
	} catch (FileNotFoundException e) {
		System.out.println("Failed to read file: " + filename);
	} catch (IOException e) {
		System.out.println("Failed to read file: " + filename);
	} catch (ParseException e) {
		System.out.println("Incorrectly formated JSON for file: " + filename);
	}
}
