...
public class Line implements MapObject{
	...
	private boolean lineSegmentsIntersect(Point aStart,Point aEnd,
		...
		assert(validLine);
		double aSlope = (aEnd.getY()-aStart.getY())/(aEnd.getX() - aStart.getX()); 
		double bSlope = (bEnd.getY()-bStart.getY())/(bEnd.getX() - bStart.getX());
	...
	public double getDistance(Point aPoint){
		...
		for(int i = 0;i<points.size()-1;i++){
			Point v = new Point(points.get(i).getY() - points.get(i+1).getY(),points.get(i+1).getX() - points.get(i).getX());
			Point r = new Point(points.get(i+1).getX()-aPoint.getX(),points.get(i+1).getY()-aPoint.getY());
