...
public class ImuVisualPanel extends JPanel {
    ...
    private void drawSide(Graphics2D g2d, double[] i1, double[] i2) {
        ...
        p1[0] *= p1[2];
        p1[1] *= p1[2]; //If larger z, scale the x and y so it looks like it's in perspective
        ...
        p2[0] *= p2[2];
        p2[1] *= p2[2]; //Scale x and y

        //After scaling, center the drawing
        p1[0] += 0.5 * frameWidth;
        p2[0] += 0.5 * frameWidth; //Remember, +y is down...
        p1[1] += 0.5 * frameHeight;
        p2[1] += 0.5 * frameHeight;
